...
def get_summary(db: Session = Depends(get_db), current_user: User = Depends(get_current_manager_or_admin)):
    return dashboard_service.get_dashboard_summary(db, current_user)

@router.get("/delays", response_model=List[DelayedTaskDetail])
def get_delays(db: Session = Depends(get_db), current_user: User = Depends(get_current_manager_or_admin)):
    return dashboard_service.get_delayed_tasks(db, current_user)

@router.get("/progress/clients", response_model=List[ClientProgress])
def get_all_progress(db: Session = Depends(get_db), current_user: User = Depends(get_current_manager_or_admin)):
